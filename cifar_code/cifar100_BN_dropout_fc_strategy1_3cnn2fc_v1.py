...
def CNN():
    ...
    conv1=tf.nn.relu(tf.nn.bias_add(conv1_bn,bias1))
    ...
    conv2=tf.nn.relu(tf.nn.bias_add(conv2_bn,bias2)) 
    ...
    fc2_drop=tf.nn.dropout(fc1_relu, keep_prob)
    ...
    for step in range(max_steps):
        ...
        train_op.run(feed_dict={image_holder:image_batch,label_holder:label_batch,keep_prob:0.5,is_training:True,
                                global_step:0})
    ...
    while step<num_iter:
        ...
        correction=sess.run([correction_prediction],feed_dict={image_holder:test_data,label_holder:test_label, keep_prob:1, 
                                                               is_training:False,global_step:1})
